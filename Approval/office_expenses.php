...
$projectParam = isset($_GET['project_id']) ? $_GET['project_id'] : null;

if ($projectParam === "OE") {
    // Office Expenses only (no join to projects, dahil project_id = 0)
    $sql = "SELECT id, supplier_name, tin_number, invoice_number, particulars, amount, created_at, status, released_date, group_number
            FROM office_expenses
            WHERE status = 'released'
            ORDER BY created_at DESC";
    $project_name = "Office Expenses";
} else {
    $project_id = intval($projectParam);
    if ($project_id > 0) {
        $sql = "SELECT oe.id, p.project_name, oe.supplier_name, oe.tin_number, oe.invoice_number, 
                       oe.particulars, oe.amount, oe.created_at, oe.status, oe.released_date, oe.group_number
                FROM office_expenses oe
                JOIN projects p ON oe.project_id = p.id
                WHERE oe.project_id = $project_id
        ...
        // Fallback - show all projects' office expenses
        $sql = "SELECT oe.id, p.project_name, oe.supplier_name, oe.tin_number, oe.invoice_number, 
                       oe.particulars, oe.amount, oe.created_at, oe.status, oe.released_date, oe.group_number
                FROM office_expenses oe
                JOIN projects p ON oe.project_id = p.id
...
<head>
    <meta charset="UTF-8">
    <title>Office Expenses | Admin Panel</title>
...
<div class="container content">
    ...
    <?php if ($successMessage): ?>
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: 'The office expense has been successfully added.',
    ...
    <h3 class="text-center">Office Expenses for <?= htmlspecialchars($project_name) ?></h3>
        <table>
            <thead>
                <tr>
                    <th>Supplier Name</th>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="text-center">No office expenses found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
