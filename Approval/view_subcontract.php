...
// Fetch sub-contracts and join with the projects table to get project name
$project_id = isset($_GET['project_id']) ? intval($_GET['project_id']) : 0;

if ($project_id > 0) {
    $sql = "SELECT sc.id, p.project_name, sc.supplier_name, sc.contact_number, sc.tcp, sc.particular, sc.category, sc.created_at
...
<head>
    <meta charset="UTF-8">
    <title>Sub-Contracts | Admin Panel</title>
...
@media (max-width: 768px) {
    ...
    table {
        font-size: 13px;
        min-width: 600px; /* horizontal scroll for small screens */
    }
...
<div class="container content">
    ...
    <?php if ($successMessage): ?>
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: 'The sub-contract has been successfully added.',
    ...
    <div class="table-container">
        <h3 class="text-center">Sub-Contracts for <?= htmlspecialchars($project_name) ?></h3>
        <table>
            <thead>
                <tr>
                    <th>Project</th>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($row['category']) ?></td> <!-- Display the category -->
                            <td><?= date('F d, Y h:i A', strtotime($row['created_at'])) ?></td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="text-center">No sub-contracts found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
